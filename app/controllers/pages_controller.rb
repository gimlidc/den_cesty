class PagesController < ApplicationController
  ...
  def actual
    ...
    if ((Time.now > $dc.start_time) && ($dc.id !=30)) || (($dc.id ==30) && (Time.now > $dc.start_time + 1.days))
      ...
      @registered = ActiveRecord::Base.connection.execute("select reg.id, reg.name, reg.goal, reg.surname, result.official, reg.updated_at, reg.confirmed from (select walkers.id as id, walkers.name as name, walkers.surname as surname, registrations.updated_at as updated_at, registrations.confirmed as confirmed, registrations.goal as goal from registrations join walkers on (registrations.walker_id = walkers.id) where registrations.canceled = false and registrations.dc_id = "+$dc.id.to_s+") as reg left outer join (select MAX(results.official) as official, results.walker_id from results group by walker_id) as result on (reg.id = result.walker_id) order by result.official DESC, reg.surname, reg.name");
